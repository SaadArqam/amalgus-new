    ...
    <div className="min-h-screen bg-white pt-24 pb-20">
      <div className="max-w-4xl mx-auto px-4">
        ...
        <div className="text-center mb-12">
          <div className="inline-flex items-center px-4 py-1 rounded-full bg-amber/10 text-amber text-[10px] font-black uppercase tracking-widest mb-6">
            AI-Powered Recommendations
          </div>
          <h1 className="text-3xl sm:text-5xl font-black text-navy mb-6 tracking-tight leading-tight">
            Find Your <span className="text-amber">Perfect Glass</span> in Seconds
          </h1>
        ...
        <div className="relative mb-12 flex flex-col items-center">
          <textarea 
            className="w-full p-6 sm:p-8 text-lg sm:text-xl bg-gray-50 border-2 border-gray-100 rounded-[24px] sm:rounded-[32px] focus:ring-4 focus:ring-amber/20 focus:border-amber outline-none transition-all shadow-sm h-48 sm:h-56 resize-none placeholder:text-gray-300"
          ...
          <button 
            onClick={() => handleMatch()}
            disabled={loading}
            className="mt-4 sm:absolute sm:bottom-6 sm:right-6 w-full sm:w-auto bg-navy text-white px-8 py-4 rounded-2xl font-bold flex items-center justify-center gap-2 hover:bg-amber hover:text-navy transition-all shadow-xl disabled:opacity-50"
          >
            {loading ? (
          ...
          <div className="flex flex-wrap justify-center gap-2 sm:gap-3 px-2">
            {sampleQueries.map((q, i) => (
              <button 
                ...
                className="px-4 py-2 bg-white border border-gray-100 rounded-full text-xs font-medium text-gray-600 hover:border-amber hover:text-amber transition-all"
              >
                {q}
              </button>
            ))}
        ...
        {results.length > 0 && (
          <div className="space-y-8 animate-in fade-in slide-in-from-bottom-5 duration-700">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {results.map((result, i) => (
                <div key={i} className="glass-card p-8 rounded-[32px] border border-gray-100 hover:shadow-2xl transition-all relative overflow-hidden group">
                  <div className="absolute top-0 right-0 w-24 h-24 bg-amber/5 blur-2xl rounded-full"></div>
                  
                  <h3 className="text-2xl font-black text-navy mb-4 group-hover:text-amber transition-colors">
                    {result.name}
                  </h3>
                  ...
                  <button 
                    ...
                    className="w-full bg-navy text-white py-4 rounded-xl font-bold hover:bg-amber hover:text-navy transition-all shadow-lg"
                  >
            ...
            <div className="p-6 bg-amber/5 rounded-2xl mt-8 text-center border border-amber/10">
