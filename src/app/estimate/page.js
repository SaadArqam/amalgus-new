            ...
            <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100">
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <select name="glassTypeId" value={formData.glassTypeId} onChange={handleInputChange} className="w-full p-4 bg-gray-50 border border-gray-100 rounded-xl outline-none focus:ring-2 focus:ring-amber font-bold text-navy">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input type="number" name="width" value={formData.width} onChange={handleInputChange} className="w-full p-4 bg-gray-50 border border-gray-100 rounded-xl outline-none focus:ring-2 focus:ring-amber font-bold text-navy"/>
                  </div>
                  <div>
                    <label className="block text-xs font-black uppercase text-gray-400 mb-2">Height (mm)</label>
                    <input type="number" name="height" value={formData.height} onChange={handleInputChange} className="w-full p-4 bg-gray-50 border border-gray-100 rounded-xl outline-none focus:ring-2 focus:ring-amber font-bold text-navy"/>
                  </div>
                </div>
                <div>
                  <label className="block text-xs font-black uppercase text-gray-400 mb-2">Quantity (Panels)</label>
                  <input type="number" name="quantity" value={formData.quantity} onChange={handleInputChange} className="w-full p-4 bg-gray-50 border border-gray-100 rounded-xl outline-none focus:ring-2 focus:ring-amber font-bold text-navy"/>
                </div>
                <div>
                  <label className="block text-xs font-black uppercase text-gray-400 mb-2">Selected Customer Role</label>
                  <div className="w-full p-4 bg-amber/10 border border-amber/20 rounded-xl font-black text-navy">
                    {role || 'Homeowner'}
                  ...
                  <div className="space-y-3">
                    {alliedProducts.map(product => (
                      <label key={product.id} className="flex items-center gap-3 cursor-pointer group">
                        <input type="checkbox" value={product.id} checked={formData.selectedAllied.includes(product.id)} onChange={handleInputChange} className="w-5 h-5 rounded border-gray-300 text-amber focus:ring-amber cursor-pointer" />
          ...
          <div className="flex-1">
            <div className="bg-white rounded-[40px] shadow-2xl overflow-hidden border border-gray-100 print:shadow-none print:border-none">
              <div className="bg-navy p-12 text-white flex justify-between items-start">
                <div><div className="text-3xl font-black mb-2 tracking-tighter">GlassIQ Estimate</div><p className="text-amber text-sm font-bold uppercase tracking-widest">Architectural Glass Solution</p></div>
              ...
              <div className="p-12 space-y-12">
                ...
                <div className="flex justify-end pt-8">
                  <div className="w-full md:w-80 space-y-4">
                    ...
                    {calculations?.discountAmount > 0 && (
                      <div className="flex justify-between text-sm text-green-600 font-bold bg-green-50 px-4 py-2 rounded-lg"><span>{role || 'Homeowner'} Discount</span><span>- ₹{calculations?.discountAmount}</span></div>
                    )}
                    ...
                    <div className="flex justify-between text-2xl font-black text-navy border-t-2 border-navy pt-4"><span>Grand Total</span><span className="text-amber underline decoration-navy underline-offset-8">₹{calculations?.total}</span></div>
                  </div>
                </div>
                ...
                <div className="flex flex-col sm:flex-row gap-4 pt-12 print:hidden">
                  ...
                  <button onClick={() => setShowModal(true)} className="flex-1 bg-amber text-navy px-8 py-5 rounded-2xl font-black uppercase tracking-widest text-xs hover:scale-105 transition-all shadow-lg flex items-center justify-center gap-3">Request Formal Quote</button>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="bg-white rounded-[40px] p-10 max-w-md w-full relative z-10 shadow-2xl animate-in zoom-in-95 duration-300">
            ...
            <form className="space-y-6" onSubmit={(e) => { e.preventDefault(); showToast("Formal request sent to 3 local factories!"); setShowModal(false); }}>
              <div><label className="block text-[10px] font-black uppercase text-gray-400 mb-2">Full Name</label><input required className="w-full p-4 bg-gray-50 rounded-xl outline-none focus:ring-2 focus:ring-amber font-bold" /></div>
              <div><label className="block text-[10px] font-black uppercase text-gray-400 mb-2">WhatsApp Number</label><input required className="w-full p-4 bg-gray-50 rounded-xl outline-none focus:ring-2 focus:ring-amber font-bold" /></div>
              <button type="submit" className="w-full bg-navy text-white py-5 rounded-2xl font-black uppercase tracking-widest text-sm hover:bg-amber">Send Fast Request</button>
            </form>
          </div>
        </div>
      )}
